...
describes what users get. Pick a format with `-o`, or let alonhadat choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
alonhadat <command> -o table   # aligned columns for reading
alonhadat <command> -o jsonl   # one JSON object per line, for piping
alonhadat <command> -o json    # a single JSON array
alonhadat <command> -o csv     # spreadsheet friendly
alonhadat <command> -o tsv     # tab-separated
alonhadat <command> -o url     # just the URL column
alonhadat <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
alonhadat <command> --fields id,title,url
```
...
```bash
alonhadat <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
alonhadat <command>            # a table, because this is a terminal
alonhadat <command> | wc -l    # JSONL, because this is a pipe
```
